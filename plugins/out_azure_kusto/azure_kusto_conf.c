...
static int azure_kusto_resolve_cloud_endpoints(struct flb_azure_kusto *ctx)
{
    ...
    else if (strcasecmp(ctx->cloud_name, "AzureGermanCloud") == 0) {
        ctx->cloud_type = FLB_AZURE_CLOUD_GERMAN;
        scope = FLB_AZURE_KUSTO_SCOPE_GERMAN;
        resource = FLB_AZURE_KUSTO_RESOURCE_GERMAN;
        login_host = FLB_AZURE_LOGIN_HOST_GERMAN;
    }
    else {
        flb_plg_error(ctx->ins,
                      "Unknown cloud_name '%s'. Use a well-known cloud name "
                      "('AzureCloud', 'AzureChinaCloud', 'AzureUSGovernmentCloud', "
                      "'AzureGermanCloud') or specify custom endpoints via "
